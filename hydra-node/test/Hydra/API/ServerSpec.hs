...
import Test.QuickCheck (checkCoverage, cover, generate, listOf1)
...
spec = parallel $ do
  ...
  it "does not echo history if client says so" $
    checkCoverage . monadicIO $ do
      outputs :: [ServerOutput SimpleTx] <- pick (listOf1 arbitrary)
      monitor $ cover 0.1 (null outputs) "no message when reconnecting"
      monitor $ cover 0.1 (length outputs == 1) "only one message when reconnecting"
      monitor $ cover 1 (length outputs > 1) "more than one message when reconnecting"
      run . failAfter 5 $ do
        withFreePort $ \port ->
          withAPIServer @SimpleTx "127.0.0.1" (fromIntegral port) alice mockPersistence nullTracer noop $ \Server{sendOutput} -> do
            mapM_ sendOutput outputs
            withClient port "/?history=0" $ \conn -> do
              traceShowM "received start"
              received <- replicateM (length outputs + 1) (receiveData conn)
              traceShowM "received end"
              traceShowM received
              case traverse Aeson.eitherDecode received of
                Left{} -> failure $ "Failed to decode messages:\n" <> show received
                Right timedOutputs ->
  ...
  it "does not send history if client specifies history=0" $
    checkCoverage . monadicIO $ do
      outputs :: [ServerOutput SimpleTx] <- pick (listOf1 arbitrary)
      monitor $ cover 0.1 (null outputs) "no message when reconnecting"
      monitor $ cover 0.1 (length outputs == 1) "only one message when reconnecting"
      monitor $ cover 1 (length outputs > 1) "more than one message when reconnecting"
      traceShowM "arbitrary outputs"
      traceShowM outputs
      run . failAfter 5 $ do
        withFreePort $ \port ->
          withAPIServer @SimpleTx "127.0.0.1" (fromIntegral port) alice mockPersistence nullTracer noop $ \Server{sendOutput} -> do
            ...
            withClient port "/?history=0" $ \conn -> do
              ...
              case traverse Aeson.eitherDecode received of
                Left{} -> failure $ "Failed to decode messages:\n" <> show received
                Right timedOutputs ->
                  -- we only expect to see the greeting on client side
                  traceShow "here" $
                    traceShow (output <$> timedOutputs) $
                      (output <$> timedOutputs) `shouldBe` [greeting]
...
withClient port path action = do
  failAfter 5 retry
 where
  retry = runClient "127.0.0.1" port path action `catch` \(e :: IOException) -> traceShow e $ retry
